...
Wird ein auf einen DFA (welcher einen regulären Ausdruck beschreibt) ein Input
angewendet und der DFA akzeptiert diesen Input, so matcht die Eingabe mit dem
regulären Ausdruck.
...
Leider kamen später Autoren neuer Sprachstile auf die Idee auch neue Features in den regulären Ausdrücken zu verwenden.
...
Wieder zum xref:4regex-statechart.adoc[Beispiel RegEx Statechart Builder]  oder zurück zur xref:index.adoc[Übersicht].
